...
func parseEvse(measurement string, deviceType string, deviceId string, direction string, etc string, message string) string {
	...
	if direction == "up" {
		...
		if evseUp.ChargePointId == "BRIMTS01" {
			switch evseUp.ConnectorId {
			case "0":
				evseUp.EvseId = "BRIMTS01"
			case "1":
				evseUp.EvseId = "BRIMTE19400577"
			case "2":
	...
	if direction == "down" {
		...
		if evseDown.DeviceId == "BRIMTS01" {
			...
			case "1":
				evseDown.EvseId = "BRIMTE19400577"
			case "2":
